...
data_paris  = pd.read_csv('/Users/camille/repo/projet_perso/loyer/data/logement-encadrement-des-loyers.csv', sep=';')
...
df_filtered = data_paris_2022[(data_paris_2022['Nombre de pièces principales'] == select_piece) & (data_paris_2022['Epoque de construction'] == select_epoque) & (data_paris_2022['Nom du quartier'] == select_quartier) & (data_paris_2022['Type de location'] == select_loc)]

loyer = df_filtered[['Loyers de référence majorés', 'Loyers de référence', 'Loyers de référence minorés']]

st.dataframe(loyer)
input_m2 = st.number_input('Surface en m2', min_value=9, max_value=1000, value=20, step=1)
input_m2 = int(input_m2)
your_loyer = st.text_input('Votre loyer', value=0)
your_loyer = int(your_loyer)
max = loyer['Loyers de référence majorés'] * input_m2
